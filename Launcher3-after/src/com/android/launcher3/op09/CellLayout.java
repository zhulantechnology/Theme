...
public class CellLayout extends ViewGroup implements BubbleTextShadowHandler {
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (lp.cellX >= 0 && lp.cellX <= mCountX - 1 && lp.cellY >= 0 && lp.cellY <= mCountY - 1) {
            ...
            ///M: CR ALPS02343076 TODO:Need avoid duplicate view add.
            try {
     ...
     *
     * @param touchX the x point position
     * @param touchY the y point position
     */
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if ((child.getVisibility() == VISIBLE || child.getAnimation() != null)
                ...
                if (frame.contains(x, y)) {
                    ...
                    cellInfo.mPos = cellInfo.cellY * mCountX + cellInfo.cellX;
                    found = true;
                    break;
                }
            }
    ...
    private boolean pushViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
        ...
        if (!fail && clusterRect.left >= 0 && clusterRect.right <= mCountX
            && clusterRect.top >= 0 && clusterRect.bottom <= mCountY) {
            foundSolution = true;
        } else {
    ...
    private ItemConfiguration findReorderSolution(int pixelX, int pixelY, int minSpanX,
    ...
    private ItemConfiguration findConfigurationNoShuffle(int pixelX, int pixelY, int minSpanX,
    ...
    */
    ...
    void onDragEnter() {
        if (LauncherLog.DEBUG_DRAG) {
            LauncherLog.d(TAG, "onDragEnter: mDragging = " + mDragging);
        }
        mDragging = true;
    }
    ...
    void onDragExit() {
        if (LauncherLog.DEBUG_DRAG) {
            LauncherLog.d(TAG, "onDragExit: mDragging = " + mDragging);
        }
    ...
    void onDropChild(View child) {
        if (LauncherLog.DEBUG_DRAG) {
            LauncherLog.d(TAG, "onDropChild: child = " + child);
        }
  ...
  /*** M: Added to Clear all the animator listeners.
     */
    public void clear() {
        int count = mDragOutlineAnims.length;
        for (int i = 0; i < count; i++) {
            mDragOutlineAnims[i].getAnimator().removeAllListeners();
            mDragOutlineAnims[i].getAnimator().removeAllUpdateListeners();
        }
    }
}
