...
public class Launcher extends Activity
    ...
    protected void onResume() {
        ...
        //add chenshu 20160906 start
	   if("1".equals(SystemProperties.get("ro.qmobile_sales_tracker"))){
	       TelephonyManager tm = (TelephonyManager)getSystemService(Context.TELEPHONY_SERVICE);
	       SalesUtil salesUtil = new SalesUtil(this);
	       //Log.i("SalesActivity","tm.getSimState():"+tm.getSimState()+" salesUtil.getMode():"+salesUtil.getMode()+" salesUtil.getBlockMCC460():"+salesUtil.getBlockMCC460()+" tm.getSimOperator().startsWith(460):"+tm.getSimOperator().startsWith("460")+ " ActivityUtils.whetherRegister():"+ActivityUtils.whetherRegister()+ " salesUtil.getFirstBootWithSIM():"+salesUtil.getFirstBootWithSIM());
		   // if insert SIM card,
		   if (tm.getSimState() == TelephonyManager.SIM_STATE_READY && salesUtil.getMode() 
	                && (salesUtil.getBlockMCC460() && !tm.getSimOperator().startsWith("460"))
	                && (ActivityUtils.whetherRegister() != 1)
	                && salesUtil.getFirstBootWithSIM()
	           ) {
				    Intent showDialog = new Intent("com.android.sales.RegisgerDialog");
	                showDialog.addCategory(Intent.CATEGORY_DEFAULT);
				    showDialog.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
	                Log.i("SalesActivity","open");
				    this.startActivity(showDialog);
				    salesUtil.setFirstBootWithSIM(false);
			}
		}
       //add chenshu 20160906 end
    }
